...
class NumberGuessing:
    ...
    def draw(self):
        ...
        if self.reqBit >= 7:
            ...
            for num in cells.keys():
                ...
                text_rect = text.get_rect(
                    center=(cells[num][0] * 80 + 40, cells[num][1] * 55 + 27)
                )
        ...
        resized_screen = pygame.transform.scale(
            self.orig_screen,
        ...
        while self.is_running:
            ...
            for event in self.py_events:
                if event.type == pygame.QUIT:
                    ...
                    if self.reset_button.check_clicked(
                        pygame.mouse.get_pos(), self.screen
                    ):
                        self.reqBit = 0
                        self.res = 0
                        self.draw()
                    elif (
                        self.yes_button.check_clicked(
                            pygame.mouse.get_pos(), self.screen
                        ) and self.reqBit < 7
                    ):
                        self.input_received(1)
                    elif (
                        self.no_button.check_clicked(
                            pygame.mouse.get_pos(), self.screen
                        ) and self.reqBit < 7
                    ):
